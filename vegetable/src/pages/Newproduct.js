...
const Newproduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (name && image && category && price) {
      const fetchData = await fetch(`${process.env.REACT_APP_SERVER_DOMIN}https://veggie-arf-uvgi.onrender.com/uploadProduct`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className='p-4'>
      <form className='m-auto w-full max-w-md shadow flex flex-col p-3 bg-white' onSubmit={handleSubmit}>
        ...
        <button className='bg-blue-500 hover:bg-blue-700 text-white font-medium my-2 drop-shadow'>ADD</button>
      </form>
    </div>
  )
}
